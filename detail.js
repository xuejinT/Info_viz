...
d3.dsv('\\', './data/US_final.csv').then(function(dataset) {
	...
	//draw parallel coordinate
 	...
 	svg_a
 	  ...
 	  .attr("d",  path)
 	  .attr("class","p_line")
    .attr("id",function(d){return d.video_id.replace(/_/g, '').replace(/-/g, '')});
 	// Draw the a_ais:
...
function clearselected(){
  ...
  d3.select(".wordselected").style("fill", "#85C0CE").style("fill-opacity", ".3");
...
function highlightselectedvideo(){
  ...
  tags.forEach(element =>
        d3.select("#"+element.replace(/ /g, '').replace(/&/g, ''))
          ...
          .attr("class","wordselected")
    );
  var id = selectedvideo.video_id.replace(/_/g, '').replace(/-/g, '')
  d3.select("#"+id).attr("class","lineselected").moveToFront();
}
