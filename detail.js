...
//  - 有时bar会画出界限
//wordle
//  - 如果brushed太多似乎不能画全所有tag，考虑设置重复次数
//  - 带特殊符号的id不能作选择器，highlight会有遗漏
//parallel coordinate
//  - 线太密的地方看不到线
...
function getallattitude(dataset, brushedvideos){
  for (i=0; i<brushedvideos.length; i++){
      var id = brushedvideos[i];
      attitudeobject = {num:i, video_id:id,views:0,likes:0,dislikes:0}; // id - views - likes - dislikes
...
function getlayoutdata(selectedattitudedata){
		layoutdata=['views','likes','dislikes'].map(function(key,i){
	    return selectedattitudedata.map(function(d,j){
	        return {x: d['time'], y: d[key] };
	    })
	})
		layoutdata[0].forEach(el => el["type"]= "views");
		layoutdata[1].forEach(el => el["type"]= "likes");
		layoutdata[2].forEach(el => el["type"]= "dislikes");
		for (var x = 0 ; x < 3 ; x++){
...
d3.dsv('\\', './data/US_final.csv').then(function(dataset) {
  ...
  getallattitude(dataset, brushedvideos); //data = attitudedata
  //draw parallel coordinate
  // Extract the list of dimensions we want to keep in the plot. Here I keep all except the column called Species
  dimensions = d3.keys(attitudedata[0]).filter(function(d) { return d != "video_id" &&  d !="num"})
  ...
  y_l = d3.scaleLinear()
        .range([140,0])
        .domain([0,1.2 * d3.max(selectedattitudedata,function(d){return d.views})])
  y_r = d3.scaleLinear()
        .range([140,240])
        .domain([0,1.2 * d3.max(selectedattitudedata,function(d){return d.likes})])
  //bar
  ...
  var layer = svg_s.selectAll(".stack")
  				...
  				.enter()
  				.append("g")
  				.attr('class', function(d,i){return 'stacktype'+i;})
  				.style("fill",function(d,i){
  					return color_s[i];
  				});
  //views bar
  layer.selectAll(".stacktype0")
  	.data(function(d){return d})
  	.enter()
  	.append("rect")
  	.filter(function(d){return d.type == "views" })
  	.attr("x",function(d){return bandScale(d.bandvalue)})
  	.attr("y",function(d){
  		return y_l(d.y)})
  ...
  layer.selectAll(".stacktype1,.stacktype2")
  	...
  	.attr("width", bandScale.bandwidth())
  	.attr('transform', 'translate(34,0)')
  //add date label as scale
  layer.selectAll(".stacktype0")
  	.data(function(d){return d})
  	.enter()
  	.append("text")
  	.attr("class","datelabel")
  	.attr("x",function(d){return bandScale(d.bandvalue) + 0.5 * bandScale.bandwidth()-14})
  	.attr("y",10)
  	.text(function(d){return formatTime(d.x)})
...
function updatelinechart(){
  ...
  dateExtent = d3.extent(selectedattitudedata, function(d) {
    return +d.time;});
  x_s.domain(dateExtent);
  timeaxis = d3.axisBottom(x_s)
        .tickFormat(d3.timeFormat("%d %b"))
        .ticks(d3.timeDay)
  y_l.domain([0,1.2 * d3.max(selectedattitudedata,function(d){return d.views})])
  y_r.domain([0,1.2 * d3.max(selectedattitudedata,function(d){return d.likes})])
  
  d3.selectAll(".stacktype0,.stacktype1,.stacktype2").remove();
  	//redraw
  color_s = ["#1C7E8C","#9D4747","#5D8931"]
  var layer = svg_s.selectAll("rect")
  				.data(layoutdata)
  				.enter()
  				.append("g")
  				.attr('class', function(d,i){return 'stacktype'+i;})
  ...
  var svg_s_transition = d3.select("#selectedattitude").transition();
  svg_s_transition.select(".x.axis")
        .duration(750)
        .call(timeaxis);
      //y axis
  svg_s_transition.select(".yl.axis")
        .duration(750)
        .call(d3.axisRight(y_l)); 
  svg_s_transition.select(".yr.axis")
        .duration(750)
        .call(d3.axisRight(y_r)); 
  ...
  //      .x(d => x_s(d.time))
  //      .y(d => y_r(d.dislikes))
  //svg_s.select("#viewsline")
  //      .duration(750)
  //      .attr("d",viewsline(selectedattitudedata));
  //svg_s.select("#likesline")
  //      .duration(750)
  //      .attr("d",likesline(selectedattitudedata));
  //svg_s.select("#dislikesline")
  //      .duration(750)
  //      .attr("d",dislikesline(selectedattitudedata));
  });
}
