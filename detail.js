...
//selected video
//  - 标题超过两行会和channel重叠
//  - 只有一个case时，attitude没有线
//  - 有时线会画出界限
//  - x轴很奇怪，有的时候只有一个tick有label
//wordle
...
var dateExtent, x_s, timeaxis, y_l, y_r, viewsline,likesline,dislikesline;
...
d3.dsv('\\', './data/US_final.csv').then(function(dataset) {
  ...
  y_r = d3.scaleLinear()
        .range([240,0])
        .domain([0,1.5 * d3.max(selectedattitudedata,function(d){return d.likes})])
    svg_s.append('g')
        .attr('class', 'x axis')
...
function updatelinechart(){
  ...
  var svg_s = d3.select("#selectedattitude").transition();
  
  svg_s.select(".x.axis")
        .duration(750)
        .call(timeaxis);
      //y axis
  svg_s.select(".yl.axis")
        .duration(750)
        .call(d3.axisRight(y_l)); 
  svg_s.select(".yr.axis")
        .duration(750)
        .call(d3.axisLeft(y_r)); 
      //line
  viewsline = d3.line()
