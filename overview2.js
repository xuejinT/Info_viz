...
d3.dsv('\\', './data/US_final.csv').then(function(data) {
    ...
    var peak_nest = d3.nest()
      .key(function(d) { return d.video_id; })
      .rollup(function(v) { return d3.max(v, function(d) { return d.trending_date; }); })
      .entries(data);

    peak_nest.forEach(function(d){
        peak_map[d["key"]] = d["value"]
    })

    // console.log(peak_map)
...
function highlightBrushed(brushedNodes) {
  ...
  //   if(isPeak) {
        // g.selectAll('.video-clicked').filter(".temp").remove();
  //   }
    // g.selectAll('.video-clicked').filter(".temp").attr('class', 'video');
...
function highlight_all(video_obj, curr_element) {
    ...
    if(isPeak){
        highlight_hidden_circle(video_obj);
    }
}
...
function reset_all_circles(curr_element) {
    ...
    svg.selectAll(".video-hovered")
        ...
        .attr("r", radius);
    if(isPeak){
        g.selectAll('.temp').filter(".video-hovered").remove();
        // temp_item.remove();
    }
}
...
function highlight_single_video() {
    ...
    svg.selectAll('.line').remove();
    g.selectAll('.temp').attr("class", "temp");
    g.selectAll('.video-clicked,.video-hovered,.brushed').attr('class', "video");

    g.selectAll('.temp').attr("class", "temp video-clicked");
...
function update_plot(caller, args) {
    if (caller === "tag_change") {
        ...
        if(isPeak){
            ...
            videos = videos.filter(function(d){
                return d.trending_date == peak_map[d.video_id];
            });
        }else{
            var unique_ids=new Set()
            var date_range = new Set()
            videos.forEach(function(d){
                unique_ids.add(d.video_id);
                date_range.add(d.trending_date.toString());
            })

            videos = all_videos.filter(function(d){
                return unique_ids.has(d.video_id) && date_range.has(d.trending_date.toString());
            })
            console.log(videos);
        }
...
function highlight_hidden_circle(curr_video){
    // highlighted_id = ""
    var hidden_videos =  all_videos.filter(function(d){
        return d.video_id==curr_video.video_id && d.trending_date!=curr_video.trending_date;
    })
    new_videos = videos.concat(hidden_videos);
    console.log(hidden_videos)
    var existing_circles = g.selectAll('circle')
        .data(new_videos);
    var enter = existing_circles.enter()
        .append('circle')
        .attr('class', 'video-hovered temp') // Add the classname that we selected on
        .attr('r', radius)
        .attr('cx', function(d) {
            return xScale(d.trending_date);
        })
